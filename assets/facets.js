...
const BOOKSTORE_PERSISTENT_QUERY_PARAMS = new Set([
  'q',
  'type',
  'view',
  'author',
...
function shouldPreserveBookstoreQueryParam(name) {
  return BOOKSTORE_PERSISTENT_QUERY_PARAMS.has(name) || name.startsWith('options[');
}

function mergeBookstorePersistentQueryParams(targetUrl) {
...
class FacetsFormComponent extends Component {
  ...
  createURLParameters(formData = new FormData(this.refs.facetsForm)) {
    ...
    for (const [name, value] of currentParameters.entries()) {
      ...
      newParameters.append(name, value);
    }
